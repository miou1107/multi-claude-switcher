...
- **Rescan Accounts**: Maintenance → **Rescan accounts…** scans the machine for Claude accounts and opens a review page in your browser (UUID, completeness, email, Team, conversations, last-updated), then lets you check which to manage. Only **complete** (switchable) accounts can be checked — incomplete/ghost accounts (orphaned Code sessions with no login) are listed read-only.
