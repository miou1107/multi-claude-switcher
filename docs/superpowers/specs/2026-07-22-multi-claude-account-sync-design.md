...
> **Bucket-naming invariant (proven in Phase 0 — now IMPLEMENTED).** The Code tab reads **only** `claude-code-sessions/<lastKnownAccountUuid>/`, where the UUID is the target profile's logged-in account. So sync reads the **source** profile's own account bucket and re-homes those sessions under the **target** profile's `lastKnownAccountUuid` bucket, copying **only that one bucket** (foreign/orphaned buckets are deliberately left behind so the target is never re-polluted). A verbatim source-bucket copy — the original behavior — was a **silent failure**: files landed on disk but never appeared in the sidebar (the exact bug that left `Claude_Profile2` empty, see probe-results Q3/Q4). `lastKnownAccountUuid` is read from each profile's `config.json` (identity file, itself never synced) via `platform.GetProfileAccountUUID`; implemented in `core.SyncSessions`, covered by `TestSyncRebucketsIntoTargetAccount`.
>
> Side effect (accepted for Vin's self-use): re-homing means the source account's conversations become visible **under the target account** after a switch. Over switches both ways the two accounts converge to the same history. This is the intended "history follows you across accounts" behavior; it also means one account's conversations physically appear under the other (the cross-account privacy point Vin has scoped out for self-use).
...
- Repo: `github.com/miou1107/multi-claude-switcher` (public); local at `/Users/example/SourceCode/multi-claude-switcher` (standalone git repo, `main`)
