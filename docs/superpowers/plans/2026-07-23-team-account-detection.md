...
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		{"team only", []orgInfo{{Name: "Acme", Tier: "default_raven"}}, AccountTeam},
		{"team plus personal", []orgInfo{
			{Name: "x's Organization", Tier: "default_claude_ai"},
			{Name: "Acme", Tier: "default_raven"},
		}, AccountTeam},
...
func TestExtractOrgs(t *testing.T) {
	blob := `{"account":{"organizations":[` +
		`{"name":"Acme","rate_limit_tier":"default_raven","billing_type":"stripe_subscription"},` +
	...
	if orgs[0].Name != "Acme" || orgs[0].Tier != "default_raven" {
...
func TestDetectAccountType(t *testing.T) {
	team := writeFixtureLS(t, `{"organizations":[{"name":"Acme","rate_limit_tier":"default_raven","billing_type":"stripe_subscription"}]}`)
