...
- `core/emdash_test.go` — Reads this package's own source and fails on an em dash inside any `fmt.Errorf` / `errors.New` / `fmt.Sprintf` string, because every error `core` returns can be drawn on a panel screen and the renderer-side guard only ever sees the text a fixture handed it.
...
- `internal/panelui/emdash_test.go` — Renders every screen a user can land on and fails on an em dash in the visible text, tags stripped. Error fixtures carry the real message text `core` returns, not a paraphrase.
