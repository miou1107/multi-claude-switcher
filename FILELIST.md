...
- `internal/panelui/profiles.go` — Builds the account-list view models both panels render from: which profiles to show, which is running, which has an account signed in, and how many conversations it holds. Shared because each host used to keep its own copy and they drifted.
- `internal/panelui/profiles_test.go` — Tests for the above, including the regression that left the macOS sync screen unable to offer any sync.
