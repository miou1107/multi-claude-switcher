...
- `internal/panelui/render_test.go` — Tests for what the panel actually shows: Rescan preselection rules, that a profile awaiting sign-in renders as a selectable card carrying its next-step note, that a recoverable signed-out account offers Recover while a dead ghost stays read-only, that two profiles holding one account raise a merge warning, and that both the account list and Settings display the version from `core.Version` in the same `v<version>` form (never a hardcoded string).
