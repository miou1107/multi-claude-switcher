...
- `internal/panelui/switchprogress.go` — The card the panel draws over the account list while a switch runs, and for a moment after: a spinner while Claude is closed and reopened, a tick naming the account once it worked, or the failure and a Close button when it did not. Sits where the confirmation dialog sat, so pressing Switch replaces the question with the progress in place. The done card dismisses itself; the failed one waits, because an error the user never read is an error that did not get reported.
- `internal/panelui/switchprogress_test.go` — Tests for the three cards: that a nil view model leaves the list unchanged, that the working card offers neither a Close button nor a self-dismissal, that the done card names the account and escapes it (the v0.9.1 apostrophe bug), and that the failed card shows the reason, waits, and never carries success wording.
