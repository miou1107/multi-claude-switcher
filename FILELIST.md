...
- `docs/team-accounts.md` — Why a Claude Team account was wrongly believed to reject imports, what was actually happening (conversations filed under the source's organization, which the target never reads), and the measurements from both the wrong diagnosis and its correction.
...
- `cmd/mcs-menubar/main.go` — Menu-bar app: Go side of the native popover panel (buildProfiles, switch, rescan, quit; renders panel HTML into the WKWebView). Also wires the panel's add/recover/merge actions: `newProfile`/`showRecover` open the in-panel name screen, `createProfile` runs the create (looking up a ghost's sources from a fresh scan), and `showMerge`/`mergeConfirm` drive the merge screen and run it (closing Claude first). Prunes stale pending sign-ins each time the panel opens.
...
- `internal/panelui/render.go` — Shared renderer for the account panel's HTML/CSS/JS. Both macOS (`cmd/mcs-menubar`, WKWebView) and Windows (`cmd/mcs-tray --panel`, WebView2) load the exact same output, feature-detecting the JS bridge (`window.webkit.messageHandlers` vs `window.mcsAction`). Views include the account list (with the duplicate-account warning), Rescan, Settings, Rename, the name-the-profile screen (`RenderNewProfile`, shared by the add and recover paths), the merge screen (`RenderMerge`), and Sync.
- `internal/panelui/state.go` — `PanelState`: which screen is up, whether a row's rename editor is open, and whether a progress card is over the top, plus the rules for how the three interact (navigation clears the card, returning to the list via the non-navigation paths does not, the sticky notification is delivered under the lock). Held by both hosts so they cannot drift.
- `internal/panelui/state_test.go` — Tests for `PanelState`, including a deterministic check that the sticky notification is delivered while the lock is still held, which is what stops one goroutine's notification overtaking another's.
- `internal/emdash/emdash_test.go` — Test-only package. Reads every package's source and fails on an em dash in a string a user can be shown, covering copy no fixture happens to render (an error only produced when a particular call fails, a message box only shown when a runtime is missing). Replaced `core/emdash_test.go`, which checked `core` alone and so never saw the sixteen live em dashes in `platform/` and both hosts.
- `internal/hostparity/hostparity_test.go` — Test-only package. Reads both hosts' sources and fails when the macOS and Windows panel action switches stop offering the same set of actions, or when the page sends an action no host answers. The panel is one shared page, so an action handled by only one host is a button that silently does nothing on the other platform.
- `internal/panelui/progress.go` — The card the panel draws over whatever screen the user is on while a long operation runs, and for a moment after: a spinner while Claude is closed and reopened, a tick and what happened once it worked, or the failure and a Close button when it did not. `WithProgress` lays it over an already-rendered page, so every screen gets it from one call in each host rather than a view model threaded through each renderer. Shared by switching, syncing, merging and backing up, each supplying its own copy through a `*Starting`/`*Outcome` pair so the two hosts cannot word the same outcome differently. The done card dismisses itself; a card carrying a warning or a failure waits, because a message the user never read did not get reported.
- `internal/panelui/progress_test.go` — Tests for the four card shapes and the outcome helpers: that a nil view model leaves the page byte for byte unchanged, that every screen can carry the card inside its body, that the working card offers neither a Close button nor a self-dismissal, that a sync whose files could not be read is still a success with a warning rather than a failure, that the dismiss action is allowlisted rather than escaped, and that everything the caller supplies is escaped (the v0.9.1 apostrophe bug).
- `internal/panelui/profiles.go` — Builds the account-list view models both panels render from: which profiles to show (curated list, else anything signed-in, MCS-created, or pending its one-time sign-in — so a freshly created profile is never hidden, and adding one never curates away the accounts already there), which is running, which has an account signed in, the account UUID it holds (so the list can spot duplicates), and how many conversations it holds. Shared because each host used to keep its own copy and they drifted.
- `internal/panelui/profiles_test.go` — Tests for the above, including the regression that left the macOS sync screen unable to offer any sync, and that each profile carries its account UUID so the duplicate warning can fire.
- `internal/panelui/emdash_test.go` — Renders every screen a user can land on and fails on an em dash in the visible text, tags stripped, plus the dialog copy inside each page's `<script>`. Error fixtures carry the real message text `core` returns, not a paraphrase. The script pass scans the JavaScript as a lexer would rather than by regex, which is what lets it see double-quoted and template literals, a `//` inside a string, and an escaped apostrophe.
- `internal/panelui/render_test.go` — Tests for what the panel actually shows: Rescan preselection rules, that a profile awaiting sign-in renders as a selectable card carrying its next-step note, that a recoverable signed-out account offers Recover while a dead ghost stays read-only, that two profiles holding one account raise a merge warning, that the name-the-profile screen serves both the add and recover paths with the right copy, that the merge screen keeps the in-use profile by default and shows the plan's combined total (not the double-counted sum) while disclosing conflicts and unreadable files, and that both the account list and Settings display the version from `core.Version` in the same `v<version>` form (never a hardcoded string).
- `cmd/mcs-tray/panel_windows.go` — Windows-only WebView2 popup panel host (`mcs-tray.exe --panel`, long-lived: parks off-screen and shows on the tray's `SHOW x,y`, or shows immediately with `--anchor X,Y` when launched by hand): borderless topmost window, JS bridge via jchv/go-webview2 Bind, parks off-screen again on outside-click (WM_ACTIVATE WA_INACTIVE) rather than exiting, re-rendering the account list while parked so the next show is instant. Places itself next to the clicked tray icon using the anchor point and the work area of that monitor (MonitorFromRect + GetMonitorInfoW), so it follows the taskbar to any edge and onto secondary displays. Parks the window off-screen (never hidden — WebView2 stops rendering while hidden and does not resume) while go-webview2 embeds the browser, since it creates the window already visible at the default position, and moves it into view only once styled, sized and loaded. Applies `WS_EX_TOOLWINDOW` as soon as the window exists so no taskbar button ever appears for it, takes the foreground via an AttachThreadInput handover, arms the outside-click dismissal only after confirming the window reached the foreground, and logs window position/visibility/focus so a panel that starts but never appears is diagnosable. MessageBox+install-link fallback when the WebView2 Runtime is missing. Wires the same add/recover/merge panel actions as the macOS host so the two behave identically: the add card now opens the in-panel name screen (not the tray's native dialog), and a successful create asks the tray to restart its migration watcher via an `MCS_MIGRATION_QUEUED` stdout line.
- `cmd/mcs-tray/panel_other.go` — Non-Windows stub for the panel entry point (never called on darwin — macOS uses `cmd/mcs-menubar`).
- `cmd/mcs-tray/onready_windows.go` — Windows-only tray `onReady`: starts and supervises one long-lived panel process (respawning it with backoff if it dies, killing it on tray exit); left-click (`systray.SetOnTapped`) toggles the panel by sending it `SHOW x,y` on stdin with the cursor position, having first granted it the foreground right; right-click keeps a single **Quit** item as the escape hatch for when the WebView2 panel cannot open. Reads the panel's stdout for `MCS_SHOWN`/`MCS_HIDDEN` (which drive the toggle), `MCS_QUIT` (which shuts the tray down too), `MCS_NEW_PROFILE` (the tray-menu native add flow), `MCS_MIGRATION_QUEUED` (restarts the post-sign-in migration watcher after the panel creates a profile), and `MCS_CHECK_UPDATES` (runs the manual update check here, since this process owns the update lock and survives an install that replaces the executable — and it is the only manual route on Windows, the tray menu's own **Check for Updates** item being macOS-only).
...
- `platform/activeorg.go` — Reads which organization a profile is signed in to, from the per-organization `dxt:allowlistLastUpdated` stamp Claude Desktop refreshes at launch. Sessions are filed per account AND per organization, and the app reads only the signed-in one, so this is half of the address sync has to write to.
- `platform/activeorg_test.go` — Tests for that read: newest stamp wins, a single organization needs no comparison, an unreadable stamp is skipped, and an unknown organization is an error rather than a guess.
...
- `platform/windows_test.go` — Unit tests for Windows `--user-data-dir` parsing, desktop-vs-CLI process detection, path matching, and listing every running profile from a set of command lines, including a flagless main process being the default profile while a flagless child process is not.
...
- `docs/superpowers/specs/2026-08-05-switch-progress-design.md` — Design spec for the switch progress card: why the four steps the panel could have shown would have been three steps of theatre, which of `SafeSwitch`'s errors mean the switch failed and which mean it worked but the sync did not, and what clears the card.
