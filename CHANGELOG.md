...
- **Conversations an old version filed in the wrong place are cleared away.**
  Sync before v0.11.2 saved conversations under the wrong organization, where no
  account can open them. The fix that followed re-saved them correctly, so the
