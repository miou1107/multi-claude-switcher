...
**Q：（Windows）切過去、登入之後，怎麼又回到另一個帳號？**
請更新到最新版。用 Google 登入時，瀏覽器會用一個連結把結果交回 Claude。
Windows 預設會把這個連結交給你的預設帳號，除非切換器先把它導到別的帳號。
舊版只替「從來沒登入過」的帳號導，所以被要求重新登入的帳號沒被導到。
...
裡面還會有一個 `tidied-` 開頭的資料夾，放的是舊版本存錯位置、任何帳號都打不開的對話。
每個帳號保留最近五份快照，更舊的會移到旁邊的 `.trash` 資料夾，放滿 30 天才真正刪除，
