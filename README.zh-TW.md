...
- **重新掃描帳號**：維護（Maintenance）選單裡的 **Rescan accounts…**，會掃描這台機器上所有 Claude 帳號，並在瀏覽器開啟審核頁面（UUID、完整性、email、Team、對話數、最後更新時間），再讓你勾選要管理哪些帳號。只有**完整**（可切換）的帳號能被勾選；不完整／幽靈帳號（沒有登入紀錄的孤立 Code session）只會唯讀列出。
...
2. 從**開始選單**啟動 **Multi-Claude Switcher**。它會出現在系統匣（右下角，可能要點「顯示隱藏的圖示」那個箭頭）成為一對眼睛的圖示。點它打開面板，跟 macOS 是同一個面板：列出你的帳號和各自的訂閱方案、標出目前使用中的那個，點另一個帳號並確認後就切換過去。**Rescan**（重新掃描帳號）、**Sync**（同步對話）、**Rename**（改名）和 **Settings**（設定，含開機自動啟動）全部都在面板裡面。點面板以外的地方、按 **Esc**、或再點一次圖示都會關閉。對圖示按右鍵只有 **Quit**（結束）一項。App 會**自動更新**，所以你只需要安裝這一次：偵測到新版本後會在背景自己下載、自己安裝，然後用新版本重新啟動。不會問你、不會開下載頁，也不需要系統管理員權限。
