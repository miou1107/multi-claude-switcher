...
2. 從**開始選單**啟動 **Multi-Claude Switcher**。它會出現在系統匣（右下角，可能要點「顯示隱藏的圖示」那個箭頭）成為一對眼睛的圖示。點它就能切換設定檔，打勾的那個是目前正在用的。從選單開啟 **Start at Login** 可讓它開機自動啟動。有新版本時它會通知你；點 **Check for Updates** 會打開下載頁，執行新的安裝程式即可升級（會直接覆蓋舊版）。
