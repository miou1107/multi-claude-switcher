...
// autoSyncWarningMessage builds the enable-time warning.
//
// It used to add a caveat that Code conversations could not be imported into a
// Team account. That was a bug in MCS being reported as a property of the
// account: sessions are filed per account AND per organization, and sync wrote
...
func askEnableAutoSync() autoSyncChoice {
	return askEnableAutoSyncChoice(autoSyncWarningMessage())
}
